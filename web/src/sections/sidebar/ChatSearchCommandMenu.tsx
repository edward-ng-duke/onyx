...
function DynamicFooter() {
  ...
  const actionLabel =
    highlightedItemType === "filter" ? t("actionShowAll") : t("actionOpen");

  return (
    <CommandMenu.Footer
      leftActions={
        <>
          <CommandMenu.FooterAction icon={SvgArrowUpDown} label={t("actionSelect")} />
...
export default function ChatSearchCommandMenu({
  ...
  const headerFilters = useMemo(() => {
    if (activeFilter === "chats") {
      return [{ id: "chats", label: t("sessions") }];
    }
    if (activeFilter === "projects") {
      return [{ id: "projects", label: t("projects") }];
    }
    return [];
  }, [activeFilter, t]);
  ...
  return (
    <>
      <div aria-label={t("openChatSearch")} onClick={() => setOpen(true)}>
        {trigger}
      </div>
      ...
      <CommandMenu open={open} onOpenChange={handleOpenChange}>
        <CommandMenu.Content>
          ...
          <CommandMenu.List
            emptyMessage={
              hasSearchValue ? t("noResults") : t("emptyState")
            }
          >
              ...
              displayedChats.length > 0 && (
                <>
                  {searchValue.trim().length === 0 && (
                    ...
                    >
                      {activeFilter === "chats" ? t("recent") : t("recentSessions")}
                    </CommandMenu.Filter>
                  )}
            ...
            {((activeFilter === "chats" && displayedChats.length === 0) ||
              ...
              <TextSeparator text={t("noMoreResults")} className="mt-auto mb-2" />
            )}
          </CommandMenu.List>
